...
func setup(cfg *config.KubectlUI) *mux.Router {
	...
	r.HandleFunc("/get-services", h.GetServices)
	r.HandleFunc("/describe-service", h.DescribeService).Methods("POST")
	r.HandleFunc("/describe-pod", h.DescribePod).Methods("POST")
	r.HandleFunc("/describe-deployment", h.DescribeDeployment).Methods("POST")
